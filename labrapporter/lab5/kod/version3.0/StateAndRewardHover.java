...
public class StateAndRewardHover extends StateAndReward{
	...
	final int VERTICAL_STATES_CONSTANT =3;// 11
	final double LOWER_LIMIT_VERTICAL =-0.8;
	final double UPPER_LIMIT_VERTICAL=0.8;
	@Override
	public String getState(double angle, double vx, double vy) {
		...
		int statevertiVerticalVelocityDiscrete = discretize(vy, VERTICAL_STATES_CONSTANT, -4,0);//-10,10
...
//			}
		...
		if ((vy >=LOWER_LIMIT_VERTICAL) && (vy <= UPPER_LIMIT_VERTICAL))
		{
...
//			strBuilder.append("_vVelocity_SUCKS");
	...
	public double getReward(double angle, double vx, double vy) {
		...
		int stateAngleDiscrete = discretize2(angle,ANGLE_STATES_CONSTANT, -3.14159,3.14159 );
		int stateHorizontalVelocityDiscrete = discretize(vx, HORISONTAL_STATES_CONSTANT, -15,15); //-24,24
		int statevertiVerticalVelocityDiscrete = discretize(vy, VERTICAL_STATES_CONSTANT, -4,4);//-19,29

		double anglePartReward=0.0; 
		double horistontalPartReward=0.0;
		double verticalPartPartReward =0.0;
		double reward = 0;
		//cheeting narrowing it down further :-) moaha ha ha
//		if ((angle >=-0.1) && (angle <= 0.1))
//		{
//			anglePartReward  =4.5;
//		}else
...
//		}
		...
		}else
		{
		    ...
		    verticalPartPartReward =Math.pow( (1 / Math.abs(statevertiVerticalVelocityDiscrete-(VERTICAL_STATES_CONSTANT/2.0))), 2.0);//5.5
		}
