...
public class QLearningController extends Controller {
	...
	/* The takeAction function must be changed if this is modified */
	final static int NUM_ACTIONS = 7; 	
	/*stupid implementation but what to do if you don't wont to refactor*/ 
	final static int NONE = 0; 	
	final static int LEFT= 1; 	
	final static int RIGHT= 2; 	
	final static int MIDDLE= 3; 	
	
	final static int MIDDLELEFT= 4; 	
	final static int MIDDLERIGTHT = 5; 	
	final static int ALL= 6; 	
	final static int LEFTRIGHT= 7;  //currently not in use 	
	...
	void performAction(int action) {
		// possible actions {None,Left,Right,Middle, MiddleLeft, MiddleRight, All}
		/* Fire zeh rockets! */
		/* TODO: Remember to change NUM_ACTIONS constant to reflect the number of actions (including 0, no action) */
		/* TODO: IMPLEMENT THIS FUNCTION */
		//bläääääääää any nice patterns lurking around insted of all these switches factory?
		//encapsulate that which varies ok for now
		switch (action) {
		case NONE:
			resetRockets();
			break;
		...
		case MIDDLERIGTHT:
			...
			middleEngine.setBursting(true);
			break;
		case LEFTRIGHT: //currently not in use
			leftEngine.setBursting(true);
			rightEngine.setBursting(true);
			middleEngine.setBursting(false);
			break;
	...
	//StateAndReward stateAndReward = new StateAndRewardStrait();	
     StateAndReward stateAndReward = new StateAndRewardHover();	
	/* Main decision loop. Called every iteration by the simulator */
     //variable used for testing velocity 
     private double max_vx=0.0;
     private double max_vy=0.0;
     private double min_vx=0.0;
     private double min_vy=0.0;

     public void tick(int currentTime) {
		iteration++;
		//TODO snygga till
		if (printOutManualMode)
		{
			System.out.println("vx: " + vx.getValue() );
			System.out.println("vy: " + vy.getValue() );
			if (vx.getValue()  > max_vx)
				max_vx =vx.getValue();
			if (vy.getValue()  > max_vy)
				max_vy =vy.getValue();
			if (vx.getValue()  < min_vx)
				min_vx = vx.getValue();
			if (vy.getValue()  < min_vy)
				min_vy =vy.getValue();
			System.out.println("max_vx: " + max_vx );
			System.out.println("min_vx: " + min_vx );
			System.out.println("max_vy: " + max_vy );
			System.out.println("min_vy: " + min_vy );
		}
		if (!paused) {
			String new_state = stateAndReward.getState(angle.getValue(), vx.getValue(), vy.getValue());
			...
			double previous_reward = stateAndReward.getReward(previous_angle, previous_vx, previous_vy);
			action_counter = 0;
			...
			if (previous_state != null) {
				...
				//increment frequency table state action pair
...
//				double newUtility = Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction))* (previous_reward + GAMMA_DISCOUNT_FACTOR*
//						getMaxActionQValue(new_state) + Qtable.get(prev_stateaction));		
//				double updatedUtility = Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction))*(Ntable.get(prev_stateaction))* (previous_reward + GAMMA_DISCOUNT_FACTOR*getMaxActionQValue(new_state) - Qtable.get(prev_stateaction));		
				double reward = stateAndReward.getReward(angle.getValue(), vx.getValue(), vy.getValue());
				if (angle.getValue() == 0.0) {
					Qtable.put(previous_state+0  ,reward);
				} 
				double updatedUtility = Qtable.get(prev_stateaction) +  alpha(Ntable.get(prev_stateaction))*(reward + GAMMA_DISCOUNT_FACTOR*getMaxActionQValue(new_state) - Qtable.get(prev_stateaction));		
				Qtable.put (prev_stateaction, updatedUtility);
	...
	boolean printOutManualMode =false;
	public void toggleCustom1() {
		System.out.println("Custom key 1 pressed!");
		
		printOutManualMode=printOutManualMode?false:true;
	}
