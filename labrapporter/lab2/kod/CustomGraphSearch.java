...
public class CustomGraphSearch implements SearchObject {
	 ...
	 * return a solution in form of list af nodes 	
	 */
	public ArrayList<SearchNode> search(Problem p) {
		...
		// Initialize the frontier with the start
		//since frontier is empty at this point it doesn't make a difference if added to front or back
		...
		// Implement this!
		 ...
		 OrderingFunction enqueueFunction;
		 //instansietor get do dynamically modify queueing behavior thru instantiation param.
		 //this way both algorithms may be created thru super
		 if(insertFront){
			 enqueueFunction = new EnqueAtFront();
		 }else{
			 enqueueFunction = new EnqueueAtEnd();
		 }
		 
		 while (true){
			 if(frontier.isEmpty())
			 {
				 return path; //empty path signals no path exist
			 }
		 SearchNode currentNode = frontier.removeFirst();
		 //if node contains goalstate return coresponding solution
		 if (p.isGoalState(currentNode.getState())) {
			 path = currentNode.getPathFromRoot();
			 return path;
		}
		 //add node to exploared set
		 explored.add(currentNode);
		 // expand the chosen node adding the resulting nodes to frontier
		 ArrayList<GridPos> candidatesForFrontier = p.getReachableStatesFrom(currentNode.getState());
		 //only if not in frontier or explored set
		 for (GridPos candidateState : candidatesForFrontier) {
			SearchNode candidateNode = new SearchNode(candidateState,currentNode);
			if (notYetExpanded(candidateState))
			 {
				 if(p.isGoalState(candidateState))
				 {
					 path = candidateNode.getPathFromRoot();
					 return candidateNode.getPathFromRoot();
				 }
				 enqueueFunction.addNodeToQueue(candidateNode, frontier);
	...
	private boolean notYetExpanded(GridPos candidateState) {
		ArrayList<SearchNode> allNodes = getAllExpandedNodes();
		for (SearchNode expandedNode : allNodes) {
			if(candidateState.equals(expandedNode.getState()))
			return false;
		}	
